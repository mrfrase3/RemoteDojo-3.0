...
UserSchema.statics.loadUser = function( req, dojos, demo=false, removeUser ){
	return new Promise((resolve, reject) => {
    	...
    	if(req.session.user && req.session.loggedin){
        	this.findByUsername(req.session.user, (err, user)=>{
            	...
            	if(!user){
                	...
                	this.loadUser(req, dojos, demo).then(resolve).catch(reject);
                }
            	resolve([user]);
            });
        } else if(
        	...
        	dojos.findByDojoName(req.body.login_dojo, (err, dojo) => {
            	...
            	dojo.checkPassword(req.body.login_password).catch(reject).then(success => {
                	...
                	this.tempUser(dojo.dojoname, 'ninja', Date.now() + 24*60*60*1000).catch(reject).then(user => {
                		req.session.user = user.username;
                		req.session.loggedin = true;
                    	if(removeUser) setTimeout(removeUser, user.expire.getTime() - Date.now(), user.username);
                    	resolve([user]);
                    });
                });
            });
...
UserSchema.statics.animals = animals;
